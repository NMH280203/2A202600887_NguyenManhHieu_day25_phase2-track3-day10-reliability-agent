...
class ResponseCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        if best_entry is not None and best_score >= self.similarity_threshold:
            if _looks_like_false_hit(query, best_entry.key):
                self.false_hit_log.append({"reason": "date_or_number_mismatch", "query": query, "cached_key": best_entry.key})
    ...
    def similarity(a: str, b: str) -> float:
        ...
        tokens_a = tokenize(a.lower())
        tokens_b = tokenize(b.lower())
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        if best_response is not None and best_score >= self.similarity_threshold:
            if _looks_like_false_hit(query, best_key or ""):
                self.false_hit_log.append({"reason": "date_or_number_mismatch", "query": query, "cached_key": best_key})
                return None, best_score
